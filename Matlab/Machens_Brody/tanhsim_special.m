...
function [xx, yy, tt, nullcline1, nullcline2] = tanhsym_special(par, f1, f2, pausetime, doplot )
...
if (doplot)
  ...
  axis( [0 5*(1+par.iobaseline) 0 5*(1+par.iobaseline)] );
  ...
  axis( [0 par.T 0 5*(1+par.iobaseline)] );
...
for k=1:length(t)
  ...
  if ( ~mod(k,par.noiseTimeStep) )
    ...
    if (doplot)
      ...
      set( traj, 'XData', [0, (1:(k/par.noiseTimeStep))*par.noiseTimeStep], 'YData', 1000*xx )
      drawnow;
...
if ~isempty(u1); y(u1) = (tanh((xmax-3.)*0.6)+1)*fxmax; end
if ~isempty(u2);
  ind = 1+round( N * (x(u2)-xgrd(1))/ ( xgrd(end)-xgrd(1) ));
  ind( ind>N ) = N;
  %y(u2) = fx( ind );
  y(u2) = (tanh((x(u2)*1000-3)*0.6)+1)*fxmax;
end
if ~isempty(u3); y(u3) = (tanh(-3.*0.6)+1)*fxmax; end;
